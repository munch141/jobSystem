...
export interface IJobsClient {
     ...
     * @return Created
     */
    jobs(body: Job | undefined): Promise<void>;
}
...
export class JobsClient implements IJobsClient {
    ...
    protected processPending(response: Response): Promise<Job[]> {
    ...
    protected processJobs(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 201) {
            return response.text().then((_responseText) => {
...
export class WorkersClient implements IWorkersClient {
    private http: { fetch(url: RequestInfo, init?: RequestInit): Promise<Response> };
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    protected processWorkers(response: Response): Promise<Job> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
